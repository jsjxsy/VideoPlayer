package com.filemanager.lib.adapter;
...
//import com.goldplusgold.support.lib.widget.CheckBox;
...
public class ExpandableItemAdapter extends BaseMultiItemQuickAdapter<MultiItemEntity, BaseViewHolder> {
    ...
    public static final int AUDIO_VIDEO_TYPE = 0x01;
    public static final int IMAGE_TYPE = 0x02;
    public static final int DOC_TYPE = 0x03;
    public static final int OTHER_TYPE = 0x04;

    private int type;

    public ExpandableItemAdapter(int type, List<MultiItemEntity> data, boolean isPhoto) {
        super(data);
        this.type = type;
                ...
                helper.itemView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (isPhoto) {
                            ...
                            f.setIsPhoto(false);
                            if (type == AUDIO_VIDEO_TYPE) {
                                Intent intent = new Intent();
                                intent.setClassName("com.xsy.android.videoplayer", "com.xsy.android.videoplayer.activity.VideoActivity");
                                Log.e("xsy","videoPath : "+ f.getFilePath());
                                Log.e("xsy","videoTitle : "+ f.getFileName());
                                intent.putExtra("videoPath", f.getFilePath());
                                intent.putExtra("videoTitle", f.getFileName());
                        ...
                        if (f.getIsCheck()) {
                            FileDao.getInstance(mContext).insertFile(f);
//                            ((CheckBox) helper.getView(R.id.cb_file)).setChecked(true, true);
                        } else {
                            FileDao.getInstance(mContext).deleteFile(f);
