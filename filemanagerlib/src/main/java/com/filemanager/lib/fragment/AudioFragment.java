...
public class AudioFragment extends BaseFragment {
    ...
    String[] audioFormat = new String[]{"mp3", "wav", "aac", "amr"};
    ...
    public void initView() {
        ...
        mRecyclerView = (RecyclerView) getActivity().findViewById(R.id.rlv_audio);
        ReadAudioFile();
                ...
                .subscribe(
                        new Subscriber<File>() {
                            ...
                            public void onCompleted() {
                                ...
                                if (fileInfos.size() > 0) {
                                    SubItem musicItem = new SubItem("音频文件");
                                    for (int j = 0; j < fileInfos.size(); j++) {
                                        if (FileUtil.checkSuffix(fileInfos.get(j).getFilePath(), audioFormat)) {
        ...
        if (f.isDirectory()) {
            ...
            return Observable.just(f).filter(new Func1<File, Boolean>() {
                ...
                public Boolean call(File file) {
                    return f.exists() && f.canRead() && FileUtil.checkSuffix(f.getAbsolutePath(), audioFormat);
                }
            });
        }
    }
