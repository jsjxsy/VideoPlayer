...
import com.goldplusgold.support.lib.BuildConfig;


/**
 * ListView, which is capable to pin section views at its top while the rest is still scrolled.
 */
public class ListViewPinnedSection extends ListView {
            ...
            if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
                if (mSectionsDistanceY < 0) {
                    if (mPinnedSection != null && mPinnedSection.view != null) {
                        final int height = mPinnedSection.view.getHeight() + mSectionsDistanceY;
                        post(new Runnable() {
                            @Override
                            public void run() {
    ...
    public ListViewPinnedSection(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public ListViewPinnedSection(Context context, AttributeSet attrs, int defStyle) {
    ...
    void createPinnedShadow(int position) {
        ...
        View pinnedView = getAdapter().getView(position, pinnedShadow.view, ListViewPinnedSection.this);
        ...
        if (mTouchTarget != null) {
            ...
            if (action == MotionEvent.ACTION_UP) { // perform onClick on pinned view
                ...
                if (Math.abs(y - mTouchPoint.y) > mTouchSlop) {
                    ...
                    // cancelAll sequence on touch target
