package com.goldplusgold.support.lib.widget.listview;
...
public class ListViewScrollBarPanel extends ListView implements OnScrollListener {
    ...
    private boolean isHeaderViewAreaScrolling = true;
    ...
    private final Runnable mScrollBarPanelFadeRunnable = new Runnable() {
        ...
        @Override
        public void run() {
            if (mOutAnimation != null && isNeedAnimation) {
    ...
    public ListViewScrollBarPanel(Context context) {
        this(context, null);
    }

    public ListViewScrollBarPanel(Context context, AttributeSet attrs) {
        this(context, attrs, android.R.attr.listViewStyle);
    }

    public ListViewScrollBarPanel(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        super.setOnScrollListener(this);

        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ListViewScrollBarPanel);
        final int scrollBarPanelLayoutId = a.getResourceId(R.styleable.ListViewScrollBarPanel_scrollBarPanel, -1);
        final int scrollBarPanelInAnimation = a.getResourceId(R.styleable.ListViewScrollBarPanel_scrollBarPanelInAnimation, R.anim.in_animation);
        final int scrollBarPanelOutAnimation = a.getResourceId(R.styleable.ListViewScrollBarPanel_scrollBarPanelOutAnimation, R.anim.out_animation);
        a.recycle();
    ...
    public void setNeedAnimation(boolean isNeedAnimation) {
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        if (null != mPositionChangedListener && null != mScrollBarPanel) {
            ...
            if (headerCount > firstVisibleItem) {
                isHeaderViewAreaScrolling = true;
            } else {
                isHeaderViewAreaScrolling = false;
            }
...
//                mScrollBarPanelPosition = thumbOffset - mScrollBarPanel.getMeasuredHeight() / 2;
    ...
    protected boolean awakenScrollBars(int startDelay, boolean invalidate) {
        ...
        if (!isHeaderViewAreaScrolling && isAnimationPlayed == true && mScrollBarPanel != null) {
            if (mScrollBarPanel.getVisibility() == View.GONE) {
                mScrollBarPanel.setVisibility(View.VISIBLE);
                if (mInAnimation != null && isNeedAnimation) {
    ...
    public interface OnPositionChangedListener {
        void onPositionChanged(ListViewScrollBarPanel listView, int position, View scrollBarPanel);
    }
}
